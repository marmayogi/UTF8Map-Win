...
Observe that unlike `tamil.ps`(handmade Hexadecimal strings), the `myNotoTamil.ps` is generated by a C++ program uses UTF-8 encoded strings completely hiding the hex strings. The function  **strps** produces hex strings from UTF-8 encoded string which are the same and identical as the strings present in `tamil.ps`. The futile `t42` font has suddenly become fruitful  due to **strps** funtion's mapping ability from UTF-8 to CIDs (every 2 bytes in Hex strings maps to a CID)!
