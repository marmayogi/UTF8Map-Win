...
Observe that unlike `tamil.ps`(hand made), the `myNotoTamil.ps` is generated by a C++ program uses UTF-8 encoded strings completely hiding the hex strings. The function  **strps** produces hex strings from UTF-8 string which are the same and identical as the strings present in `tamil.ps`. The futile `t42` font has suddenly become an useful one due to **strps** funtions's mapping ability from UTF-8 to CIDs (every 2 bytes in Hex strings maps to a CID)!
...
2. `UTF-8 2 byte encoding:` 2 byte encoding is identified by the presence of the bit sequence **110** in the first bit and **10** in the second bit (**110**xxxxx **10**xxxxxx). i.e. code points in the range (128-2047) are represented by two bytes.
...
2. Dispatches quad buffer (consisting of 4 Unicode Points) to function **up2cid** which maps `Unicode Points` to CIDs with the help of a `mapping table` (implemented as a single dimensional array constructed with the help of Unicode Blocks. Refer `mapunicode.h`.).
   ...
   - The fifth and final parameter of **up2cid** function is `pCID` which is a ternary array of type short. This is an output array that passes out atmost 3 CIDs corresponding to Unicode Points present in the quad array (3rd parameter). This requires more explanation as given below.
      - The ghlph `ஸ்ரீ` consists of `ஸ +  ் + ர + ீ` which corresponds to four Unicode Points U+0BB8, U+0BCD, U+0BB0 and U+0BC0 respectively. The CID for ghlph `ஸ்ரீ` is 163 and Unicode Points consumed by this glyph are four. Since all Unicode points in `quad` array are consumed, `pCntUnicode` passes out zero to the caller. The ternary array is filled with only one CID and the return value of function **up2cid** will be 1 (Number of CIDs present in the ternary array `pCID`).
      - The ghlph `க்ஷ` consists of `க + ் + ஷ` which corresponds to three Unicode Points U+0B95, U+0BCD and U+0BB7 respectively. The CID for ghlph `க்ஷ` is 76 and Unicode Points consumed by this glyph are three. The untouched Unicode points in quad array is 1 which is passed out by `pCntUnicode` to the caller. The ternary array is filled with only one CID and the return value of function **up2cid** will be 1 (Number of CIDs present in the ternary array). 
      - The ghlph `மா` consists of `ம + ா` which corresponds to two Unicode Points U+0BAE and U+0BBE respectively. The CIDs for ghlph `மா`  are 29 and 41. The Unicode Points consumed by this glyph are two. The untouched Unicode points in `quad` array is 2 which is passed out by `pCntUnicode` to the caller. The ternary array is filled with both CIDs and the return value of function **up2cid** will be 2 (Number of CIDs present in the ternary array which will never exceed 3).
      - The ghlph `லோ` consists of `ே + ல + ா` which corresponds to three Unicode Points U+0BC7, U+0BB2 and U+0BBE respectively. The CIDs for ghlph `லோ`  are 47, 33 and 41. The Unicode Points consumed by this glyph are three. The untouched Unicode points in `quad` array is 1 which is passed out by `pCntUnicode` to the caller. The ternary array is filled with 3 CIDs and the return value of function **up2cid** will be 3 (Number of CIDs present in the ternary array which will never exceed 3).
...
3. Post Processing after return from function **up2cid** is an important task. Based on the return value from **up2cid** function, it knows how many CIDs are present in the Tertiary array `pCID` and copies the CIDs into 3rd parameter `pPSOutString` (an array of characters) in hex format which will be passed subsequently to Postscript's operator `show`. Variable `cntUnicode` indicates how many Unicode Points are consumed. The untouchd Unicode Points (if any) will be recycled by **strps** function.  A value of zero means an empty quad array and begins to fill quad array with Unicode points decoded from UTF-8 string. This process repeats until  parameter `pUTF8InString` which holds UTF-8 encoded string is exhuasted completely.
